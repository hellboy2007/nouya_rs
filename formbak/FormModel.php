...
class FormModel {
    ...
    public function get_form_item($items, $value= null) {
        $attr = empty($items['attr'])? null : $items['attr'];
                ...
                switch ($items['flag']) {
                        case 'text':
                                $backstr = $this->text($items['key'], $value , $attr);
                                break;
                        case 'list':
                         $begin = 1;$end = 6;
                                $backstr = $this->genSelect($items['key'], $items['list'], $value, 'form-control');
                                break;
                        case 'password':
                                $backstr = $this->password($items['key'],  $attr);
                                break;        
                        case 'textarea':
                                $backstr = $this->textarea($items['key'],  $attr);
                                break;
                        case 'select':
                                $backstr = $this->select($items['key'],  $attr);
                                break;
                        case 'radio':
                                $backstr = $this->radio($items['key'],  $attr);
                                break;
                        default:
                                # code...
                                break;
 ...
 * @return string
 */
function genSelect($name,  $list,  $selected = 0, $class = 'form-control') {
    ...
    public function input($type, $name, $value = null, $options = array())
    {
        ...
        $class_set = $this->getClassAttribute($options);
        ...
        return '<input'.$this->html->attributes($options).' '. $class_set .'>';
    }
    ...
    public function select($name, $list = array(), $selected = null, $options = array())
    {
        ...
        return "<select{$options} class=\"form-control\">{$list}</select>";
    }
   ...
   /* protected function getControllerAction($options)
    ...
    public function getClassAttribute($attributes)
    {
        $backstr = '';
        if (!array_key_exists('class', $attributes))
        {
            $backstr = ' class="form-control"';
        }
        return $backstr;
    }
